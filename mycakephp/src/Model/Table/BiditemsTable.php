...
class BiditemsTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->scalar('description')
            ->maxLength('description', 500,'500文字以内で入力してください')
            ->requirePresence('description', 'create','商品の説明は入力必須')
            ->notEmptyString('description','商品の説明は入力必須です');

        //追加　商品画像
        $validator
            ->scalar('image')
            ->requirePresence('image','create','商品画像は登録必須です')
            ->notEmptyFile('image','画像は登録必須です'); //notEmptyが先に判定されるっぽい
